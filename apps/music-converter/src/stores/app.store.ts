...
async function sha256(buffer: ArrayBuffer) {
  let hashBuf: Uint8Array<ArrayBuffer>;
  if (crypto?.subtle?.digest) {
    hashBuf = new Uint8Array(await crypto.subtle.digest("SHA-256", buffer));
  } else {
    hashBuf = digest(buffer);
  }
  ...
  function digest(buffer: ArrayBuffer) {
    ...
    for (let i = 0; i < 8; i++) {
      hashBuf[i * 4 + 0] = (H[i]! >>> 24) & 0xff;
      hashBuf[i * 4 + 1] = (H[i]! >>> 16) & 0xff;
      hashBuf[i * 4 + 2] = (H[i]! >>> 8) & 0xff;
      hashBuf[i * 4 + 3] = H[i]! & 0xff;
    }
    return hashBuf;
  }
}
